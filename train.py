...
def main(params, logging):
    ...
    if params.model_type == "NAS_MODEL":
        ...
        criterions['speed'] = SpeedLoss(scale=ori_speed).to(device)

    speed_scheduler = SpeedScheduler(search_epoch=params.epochs,
    ...
    epoch = 0
    end_epoch = 0
    if params.model_type != 'BASIC_MODEL':
        logging.info("Enable  search", device=device)

        model.length_grad(params.length_search)
        ...
        optimizer, scheduler = trainer_preparation(model=model,
                                                   learning_rate=params.learning_rate * 5,
